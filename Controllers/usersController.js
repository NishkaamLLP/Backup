...
const createUser = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      message:
        'User is registered successfully.You will need to verify your email before signing in.',
    })
  } catch (error) {
  ...
  try {
    ...
    if (token) {
      ...
      if (userUpdatedData.password) {
        ...
        userUpdatedData.password = await hashInputData(userUpdatedData.password)
      }
